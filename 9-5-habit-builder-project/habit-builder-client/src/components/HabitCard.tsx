...
const HabitCard = ({ _id, title, creator, difficulty, description, startDate, endDate, frequency, members, detailed = false }) => {
      ...
      return (
        <>
          <article className="flex flex-row gap-3">
            <p className="flex items-end gap-2 rounded bg-neutral-100 px-2 py-1 dark:bg-neutral-900">
              <DateIcon />
            ...
            <section className="flex items-center">
              {members.map(
                (member: User, index: number) =>
                  index < 3 && (
                    <div key={index} className={`rounded-[50%] border-2 border-[#f6f6f6] p-1 dark:border-neutral-700 ${colors[index][0]}`}>
  ...
  return (
    <li
      key={_id}
      className="relative flex h-fit w-full basis-1/3 flex-col items-start justify-around gap-3 rounded-xl border-2 bg-neutral-50 p-3 dark:border-neutral-700 dark:bg-neutral-950">
      <h3 className="flex flex-row items-center text-lg font-bold text-neutral-950 dark:text-neutral-200">{title}</h3>
      {habitContentJSX()}
    </li>
  );
};
